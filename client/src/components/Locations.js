...
export default class Locations extends Component {
  ...
  searchByLocation = event => {
    ...
    const mapUni = this.props.uData.filter(uni => {
      return uni.city.toLowerCase().includes(location.toLowerCase());
    });
    // console.log(mapUni);
    const newCenter = {};
    newCenter.lat = mapUni[0].lat;
    newCenter.lng = mapUni[0].lng;
    const myMarker = mapUni.map(oneUni => {
